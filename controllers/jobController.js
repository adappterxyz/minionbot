...
// Initialize existing scripts into jobStore
const files = fs.readdirSync(SCRIPTS_DIR);
files.forEach(file => {
    if (file.endsWith('.spec.ts')) {
        const jobName = file.replace('.spec.ts', '');
        jobStore.set(jobName, {
            requiredArguments: [],
            scriptPath: path.join(SCRIPTS_DIR, file),
            created: new Date()
        });
        console.log(`Loaded job: ${jobName}`);
    }
});

const triggerJob = async (req, res) => {
    try {
        const jobName = req.body?.jobName || req.jobName;
        const arguments = req.body?.arguments || {};
        
        console.log("Available jobs:", Array.from(jobStore.keys()));
        console.log("Attempting to run job:", jobName);
